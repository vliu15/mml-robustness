...
for ckpt in ${ckpts[@]}; do
    ...
    # python test.py --log_dir logs/jtt-Pointy_Nose/stage_2 --ckpt_num $ckpt --subgroup_attributes "[\"Pointy_Nose:Male\"]" --json_name "Pointy_Nose:Male.$ckpt.test_results.json"

    python test.py --log_dir logs/jtt/stage_2 --ckpt_num $ckpt --subgroup_attributes "[\"Attractive:Eyeglasses\", \"Smiling:High_Cheekbones\"]" --json_name "Attractive:Eyeglasses_Smiling:High_Cheekbones.$ckpt.test_results.json"
done
