...
class Chestxray8(Dataset):
    """Wrapper around Chestxray8 dataset for implementing additional functionality"""

    def __init__(self, config, split: str = 'train', size="small"):
        super().__init__()
        self.root = os.path.join(config.dataset.root, "chestxray8")
        ...
        if size == "small":
            self.split_base = "cxr8_small_2_"
        else:
            self.split_base = "cxr8_"
        ...
        ##transforms.Normalize([0.48865], [0.24621])
        ...
        split_attr =  pd.read_csv(
            os.path.join(self.root, f"{self.split_base}{split}.csv"), delim_whitespace=False, header=0
        )

        self.filename = split_attr.image_index.values
        attr = split_attr.drop("image_index", axis=1)
    ...
    def __getitem__(self, index):
        ...
        image = Image.open(os.path.join(self.root, "images", self.filename[index]))
